"""Run an XGBoost experiment with the shared pipeline."""
...
def set_xgboost_seed(seed: int) -> None:
    """Seed Python and NumPy without importing Torch before XGBoost."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an XGBoost experiment.")
...
def make_model(task: str, params: dict[str, Any], seed: int, n_classes: int) -> Any:
    """Construct an XGBoost estimator for a task."""
...
def predict_outputs(model: Any, X: np.ndarray, task: str) -> tuple[np.ndarray, np.ndarray | None, float]:
    ...
    with Timer() as timer:
        if task == "classification":
            ...
            probabilities = np.asarray(model.predict_proba(X)) if hasattr(model, "predict_proba") else None
        else:
            probabilities = None
            predictions = np.asarray(model.predict(X))
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    n_classes = int(np.unique(prepared.y).size) if prepared.task == "classification" else 0
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        model = make_model(prepared.task, params, seed, n_classes)
    ...
    if best is None or best_score is None:
        raise RuntimeError("No XGBoost candidate was successfully evaluated.")
